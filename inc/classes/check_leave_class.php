...
class check_leave {
	...
	private function checkLeaveEntitlement() {
		...
		foreach($nextYrL as $nYr) {
			//find the day of the week to search for
			$dayOfTheWeek = date("w",strtotime($nYr["event_startdate_time"]));
			$sql 					= "select * from flexi_user as u
			join flexi_template as f on (u.user_flexi_template=f.template_id)
			join flexi_template_name as tn on (tn.flexi_template_name_id = f.template_name_id)
			...
			join flexi_day_times as dt on (dt.fdt_flexi_days_id = tds.days_settings_id)
			where (dt.fdt_weekday_id = ".$dayOfTheWeek. " or dt.fdt_weekday_id = 6) and u.user_id = ".$this->userId;
			$checkLeave = dl::getQuery($sql);
			$time = strtotime($nYr["event_enddate_time"]) - strtotime($nYr["event_startdate_time"]);
			$lunch = 30*60; //number of seconds in 30 mins
			if($time > 21600){ //this is the number of seconds in 6 hours when lunch is mandatory
				$time = $time - $lunch;
			}
			$checkTimeHours = 60 * 60 * date("h",strtotime($checkLeave[0]["fdt_working_time"])); //works out the number of seconds in the hour
			$checkTimeMinutes = 60 * date("i",strtotime($checkLeave[0]["fdt_working_time"])); //work out the minutes.
			$checkTime = $checkTimeHours + $checkTimeMinutes;
			if($checkTime == $time) {
				$nextDaysTaken   	+= 1;
			}elseif($checkTime/2 == $time) {
				$nextDaysTaken		+= 0.5;
			}
		}
