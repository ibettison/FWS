...
function show_subMenu($button_choice) {
    ...
    if($button_choice == "View") {
		...
		if($_SESSION["userPermissions"]["year_end"]=="true") {
            ...
            echo "<div class='sub_menu_button'>".show_menuLink($_SERVER['PHP_SELF']."?choice=".$_GET["choice"]."&subchoice=thisYearStarter", 'This years starters')."</div>";
		}
		...
		if($_SESSION["userPermissions"]["templates"]=="true") {
			echo "<div class='sub_menu_button'>".show_menuLink($_SERVER['PHP_SELF']."?choice=".$_GET["choice"]."&subchoice=resendRegistration", 'Resend Registration Email')."</div>";
				...
				while(date('Y-m-d',strtotime($date)) < date('Y-m-d',strtotime($event["event_startdate_time"]))) {
					...
					if(date('l',strtotime($date))!="Saturday") {
					    ...
					    if ($own_timesheet or $authorise) {
							...
							if($permissions[0]["permission_leave_only"] == "true" and $own_timesheet){
...
function send_registration() {
	$sql = "select u.user_id, user_name, user_email, date_deleted from flexi_user as u left outer join flexi_deleted as d on (u.user_id = d.user_id) where date_deleted is NULL ORDER BY user_name";
	$users = dl::getQuery($sql);
	foreach($users as $user) {
		$userEmails[] = $user["user_email"];
	}
	$formArr = array(array("type"=>"intro", "formtitle"=>"Resend Registration", "formintro"=>"Enter the email address to resend the registration email."),
		array("type"=>"form", "form"=>array("action"=>"index.php?func=resendRegistration","method"=>"post")),
		array("prompt"=>"Email Address", "type"=>"selection", "name"=>"email_address", "listarr"=>$userEmails, "selected"=>"", "value"=>"", "clear"=>true),
		array("type"=>"submit", "buttontext"=>"Resend Registration"),
		array("type"=>'endform'));
	$form = new forms;
	echo $form->create_form($formArr);
}

function send_registrationEmail($post) {
	include('inc/email_messages.inc');
	$sql = "select user_name, user_email from flexi_user where user_email = '".$post["email_address"]."'";
	$users = dl::getQuery($sql);
	$m = new Mail();
	$encrypto = MD5(SALT.$post["email_address"]);
	$subject = $email_5_subject;
	$bodyText = $email_5_content;
	$bodyText = str_replace("%%whoto%%", $users[0]["user_name"], $bodyText);
	$bodyText = str_replace("%%link%%", $encrypto, $bodyText);
	$recipients = array($post["email_address"]);
...
function reset_userPassword($email){
	...
	$formArr = array(array("type"=>"intro", "formtitle"=>"Reset user Password", "formintro"=>"Enter the email address and password to change the user password."),
		array("type"=>"form", "form"=>array("action"=>"index.php?func=changeUserPassword","method"=>"post")),
		array("prompt"=>"Email Address", "type"=>"selection", "name"=>"email_address", "listarr"=>$userEmails, "selected"=>$email, "value"=>"", "clear"=>true),
		array("prompt"=>"Password", "type"=>"password", "name"=>"password", "length"=>20, "value"=>"", "clear"=>true),
		array("prompt"=>"retype Password", "type"=>"password", "name"=>"password2", "length"=>20, "value"=>"", "clear"=>true),
		array("type"=>"submit", "buttontext"=>"Change User Password"),
		array("type"=>'endform'));
	$form = new forms;
	echo $form->create_form($formArr);
}
...
function changeUserPassword() {
	$passcode = MD5(SALT.$_POST["email_address"]);
	change_password($_POST, $passcode);
}
...
function save_user() {
	...
	$postarr = array(addslashes($_POST["name"]),addslashes(strtolower($_POST["email"])),$perm_id, $al_id, $time_id, $flexi_id);
...
function view_user($page=0) {
	...
	$sql = "select u.user_id, user_name, date_deleted, user_email from flexi_user as u left outer join flexi_deleted as d on (u.user_id = d.user_id) where date_deleted is NULL ORDER BY user_name ASC LIMIT $page, $rows ";
	$users = dl::getQuery($sql);
	...
	echo "<table class='table_view'>";
	echo "<tr><th>User Names</th><th>Delete</th><th>Edit</th><th>Reset<BR>Password</th></tr>";
	foreach($users as $user) {
		echo "<tr><td>".$user["user_name"]."</td><td><a href='index.php?func=deleteuser&id=".$user["user_id"]."'>delete</a></td><td><a href='index.php?func=edituser&id=".$user["user_id"]."'>edit</a></td><td><a href='index.php?func=resetUserPassword&email=".$user["user_email"]."'>reset</a></td></tr>";
	}
	echo "</table>";
	$page+=28;
	if($page > 28 ){
...
function add_event($title, $intro, $user="") {
	if(check_permission("Events")) {
		...
		$leaveOverRide = $_SESSION["userPermissions"]["override_leave_entry"];
		$selectedFixed = "";
		if($duration_type == "Fixed" and $leaveOverRide == "true") {
			array_unshift($arrDurations, "");
			$selectedFixed = "Full day";
		}
		...
		if($duration_type=="Fixed" or $duration_type == "Both") {
			...
			$formArr[]=array("prompt"=>"Event Duration", "type"=>"selection", "name"=>"duration", "listarr"=>$arrDurations, "selected"=>$selectedFixed, "value"=>"", "clear"=>true);
		}
		if($duration_type=="User definable" or $duration_type == "Both" or $leaveOverRide == "true") {
...
function save_event($userId) {
	...
	$overrideLeave						= $_SESSION["userPermissions"]["override_leave_entry"];
	//check to see if the duration is fixed that a value has been entered for event duration
	if($durationType == "Fixed" and empty($_POST["duration"]) and $overrideLeave == "false") { //need to message the user ?>
		<SCRIPT language="javascript">
...
function edit_permissions() {
	if(check_permission("Templates")) {
		...
		$formArr = array(array("type"=>"intro", "formtitle"=>"Create Permission Template", "formintro"=>"Fill out the fields below to create the permission template"), 
			...
			array("prompt"=>"Template Name", "type"=>"text", "name"=>"template_name", "length"=>40, "value"=>$permission_name[0]["permission_template_name"], "clear"=>true),
	...
	if(empty($template_check)) {
		...
		if(in_array("All", $week_day_array)) { //this array only has one element and applies to all of the days_per_week
			...
			for( $count=1; $count <= $_POST["days_per_week"]; $count++) {
				...
				$values = array($dayId[0]["fw_id"], $max, $timeVal);
				$writeLine = array_combine($fields, $values);
				dl::insert("flexi_day_times", $writeLine);
			}
			$count++;
...
function this_year_starter()
{
    echo "<h1>Starters this year</h1>";
    $newStarters = dl::select(flexi_carried_forward_live, "additional_leave < 0");

    foreach($newStarters as $newStarter)
    {
        $timesheet = dl::select(flexi_timesheet, "timesheet_id =".$newStarter["timesheet_id"]);
        if(!empty($timesheet)) {
            $user = dl::select(flexi_user, "user_id =".$timesheet[0]["user_id"]);
            if(!empty($user)){
                echo $user[0]["user_name"]."<BR>";
            }
        }
    }
}
