...
<?php
						...
						if($cp["security_password"] == MD5(SALT.$password)) {
							...
							if(!empty($getPermissions)) {
								foreach($getPermissions as $gp) {
									...
									$_SESSION["userPermissions"]=array(user=>$gp["permission_user"],templates=>$gp["permission_templates"],
										teams=>$gp["permission_teams"],team_events=>$gp["permission_team_events"],team_authorise=>$gp["permission_team_authorise"],
										events=>$gp["permission_events"],event_types=>$gp["permission_event_types"],add_time=>$gp["permission_add_time"],
										edit_time=>$gp["permission_edit_time"],edit_locked_time=>$gp["permission_edit_locked_time"],add_global=>$gp["permission_add_global"],
										override_delete=>$gp["permission_override_delete"], edit_flexipot=>$gp["permission_edit_flexipot"],
										view_user_leave=>$gp["permission_view_leave"], user_messaging=>$gp["permission_messaging"],
										view_user_timesheet=>$gp["permission_view_timesheet"],view_user_override=>$gp["permission_view_override"],
										view_reports=>$gp["permission_view_reports"], year_end=>$gp["permission_year_end"], override_leave_entry=>$gp["permission_override_leave_entry"],
										lock_override=>$gp["permission_lock"],leave_only=>$gp["permission_leave_only"]);
								}
							}
	...
	<?php }else{?>
				...
				<?php if($nextYrLeave > 0) { ?>
					...
					<div class='left_text'>
						<?php echo $nextYrLeave?> approx. day(s)
					</div>
				<?php } ?>
		<?php
		...
		if($_SESSION["loggedin"]) { 
			...
			foreach($flexi_template as $ft) {
				if(date("Y-m-d") > date("Y-m-d", strtotime($ft["end_period"]))) { //todays date is after the period end so need to update the period
					...
					$sql = "select u.user_id, t.timesheet_id, u.user_name from flexi_user as u join 
					...
					foreach($users as $user) {
					    //get the users permissions to check if they should be emailed or not depending on whether they are using the flexitime portion of the app.
                        // the permission of 'permission_leave_only' set to true indicates that flexi leave is not required.
                        $sql = "select * from flexi_user as u join flexi_permission_template_name as ptn
                                on (u.user_permission_id=ptn.permission_id) join flexi_permission_template as pt on (ptn.permission_id=pt.permission_template_name_id)
                                where u.user_id = ".$user["user_id"];
                        $userPermissions = dl::getQuery($sql);
						...
						if(!empty($cf)) { //need to move the flexi
							if($cf[0]["current_flexi"] > $ft["max_deficit"]) { 
								...
								if($timeCarried > $max_surplus and !$userPermissions[0]["permission_leave_only"]) {
								...
								$writeArr = array_combine($fieldList,$valuesArr);
								dl::insert("flexi_carried_forward",$writeArr); //this has recorded the flexitime for the user for the current period. Will be used when examining the timesheet in each period.
                                //if the permission is not a leave only user then send the email.
                                if(!$userPermissions[0]["permissions_leave_only"]) {
			...
			if($_GET["choice"] == "Add") {
                ...
                if($_GET["subchoice"] == "thisYearStarter") {
                    echo "<div class='left_body'>";
                    echo "<SCRIPT>redirect('reports.php?func=thisYearStarter');</SCRIPT>" ;
                    echo "</div>";
                }
				...
				if($_GET["subchoice"] == "resendRegistration") {
					echo "<div class='left_body'>";
					send_registration();
					echo "</div>";
				}
			...
			if($_GET["func"] == "resetUserPassword") {
				echo "<div class='left_body'>";
				reset_userPassword($_GET["email"]);
				echo "</div>";
			}
			if($_GET["func"]=="resendRegistration") {
				send_registrationEmail($_POST);
			}
			if($_GET["func"]=="changeUserPassword") {
				changeUserPassword();
			}
